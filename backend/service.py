...
class JobIntakeService:
    ...
    def analyze(
        ...
        candidates = self.history.search_candidates(run.record)
        duplicate = looks_like_duplicate(run.record, candidates)
        return run, duplicate

    def write(self, record: JobRecord, excel_path: str, sheet_name: str) -> str:
    ...
    def search_history(self, query: str, priority: str, match_direction: str, limit: int) -> list[dict]:
        return self.history.search(query=query, priority=priority, match_direction=match_direction, limit=limit)

    def history_by_id(self, record_id: int) -> dict | None:
        return self.history.get_by_id(record_id)
